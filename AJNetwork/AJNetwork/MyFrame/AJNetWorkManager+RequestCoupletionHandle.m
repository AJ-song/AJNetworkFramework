//
//  AJNetWorkManager+RequestCoupletionHandle.m
//  AJNetwork
//
...
#import "AJNetWorkManager+RequestCoupletionHandle.h"
...
@implementation AJNetWorkManager (RequestCoupletionHandle)
...
{
    return [self requestWithType:HttpRequestTypeGet
...
+ (UrlSessionTask *)POST_WithUrlString:(NSString *)urlString
                      ...
                      complectionBlock:(ComplectionBlock)complectionBlock
                         progressBlock:(DownloadProgressBlock)progressBlock
{
    return [self requestWithType:HttpRequestTypePost
                     isNeedCache:isNeedCache
                       urlString:urlString
                      parameters:parameters
                complectionBlock:complectionBlock
...
+ (UrlSessionTask *)PUT_WithUrlString:(NSString *)urlString
                     ...
                     complectionBlock:(ComplectionBlock)complectionBlock
                        progressBlock:(DownloadProgressBlock)progressBlock
{
    return [self requestWithType:HttpRequestTypePut
                     isNeedCache:NO urlString:urlString
                      parameters:parameters
                complectionBlock:complectionBlock
                   progressBlock:progressBlock];
}
                                ...
                                progressBlock:(UploadProgressBlock)progressBlock
{
    ...
    NetLog(@"******************** 请求参数 ***************************");
    NetLog(@"请求头: %@\n请求方式: %@\n请求URL: %@\n请求param: %@\n\n",AJNetManagerShare.afSessionManager.requestSerializer.HTTPRequestHeaders, @"POST",URLString, parameters);
    NetLog(@"********************************************************");
    
    UrlSessionTask *sessionTask = nil;
    sessionTask = [AJNetManagerShare.afSessionManager POST:URLString parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData)
    {
        /*! 出于性能考虑,将上传图片进行压缩 */
        [imageArray enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop)
        {
            ...
            if ([obj isKindOfClass:[PHAsset class]])
            {
                ...
                [imageManager requestImageForAsset:asset targetSize:CGSizeMake(asset.pixelWidth , asset.pixelHeight) contentMode:PHImageContentModeAspectFit options:nil resultHandler:^(UIImage * _Nullable result, NSDictionary * _Nullable info)
                {
                    ...
                    [self uploadImageWithFormData:formData
                                     resizedImage:result
                                        imageType:imageType
                    ...
                    progressBlock:(UploadProgressBlock)progressBlock
{
    ...
    [avAssetExport exportAsynchronouslyWithCompletionHandler:^{
        switch ([avAssetExport status])
        {
            ...
            {
                [AJNetManagerShare.afSessionManager POST:urlString
                                              parameters:parameters
                                 ...
                                 progressBlock:(DownloadProgressBlock)progressBlock
{
    ...
    NetLog(@"******************** 请求参数 ***************************");
    NetLog(@"请求头: %@\n请求方式: %@\n请求URL: %@\n请求param: %@\n\n",AJNetManagerShare.afSessionManager.requestSerializer.HTTPRequestHeaders, @"download",urlString, parameters);
    NetLog(@"******************************************************");

    UrlSessionTask *sessionTask = nil;
    sessionTask = [AJNetManagerShare.afSessionManager downloadTaskWithRequest:downloadRequest progress:^(NSProgress * _Nonnull downloadProgress)
    {
                               ...
                               progressBlock:(UploadProgressBlock)progressBlock
{
    ...
    NetLog(@"******************** 请求参数 ***************************");
    NetLog(@"请求头: %@\n请求方式: %@\n请求URL: %@\n请求param: %@\n\n",AJNetManagerShare.afSessionManager.requestSerializer.HTTPRequestHeaders, @"uploadFile", urlString, parameters);
    NetLog(@"******************************************************");
                      ...
                      progressBlock:(DownloadProgressBlock)progressBlock
{
    ...
    //    AFHTTPSessionManager *scc = AJNetManagerShare.afSessionManager;
    //    AFHTTPResponseSerializer *scc2 = scc.responseSerializer;
    //    AFHTTPRequestSerializer *scc3 = scc.requestSerializer;
    //    NSTimeInterval timeoutInterval = AJNetManagerShare.timeoutInterval;
    //
    //    NSString *isCache = isNeedCache ? @"开启":@"关闭";
    //    CGFloat allCacheSize = [AJNetWorkCache getAllHttpCacheSize];
    
    //    NetLog(@"\n******************** 请求参数 ***************************");
    //    NetLog(@"\n请求头: %@\n超时时间设置：%.1f 秒【默认：30秒】\nAFHTTPResponseSerializer：%@【默认：AFJSONResponseSerializer】\nAFHTTPRequestSerializer：%@【默认：AFJSONRequestSerializer】\n请求方式: %@\n请求URL: %@\n请求param: %@\n是否启用缓存：%@【默认：开启】\n目前总缓存大小：%.6fM\n", AJNetManagerShare.afSessionManager.requestSerializer.HTTPRequestHeaders, timeoutInterval, scc2, scc3, requestType, URLString, parameters, isCache, allCacheSize);
    //    NetLog(@"\n********************************************************");
    
    UrlSessionTask *sessionTask = nil;
    
    // 读取缓存
    id responseObject = [AJNetWorkCache httpCacheWithUrlString:urlString parameters:parameters];
    
    if (type == HttpRequestTypeGet)
    {
        sessionTask = [AJNetManagerShare.afSessionManager GET:URLString parameters:parameters  progress:^(NSProgress * _Nonnull downloadProgress)
        {
            
        } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject)
        {
            NSError *error = [AJNetManagerShare parseResultObject:responseObject responseError:nil];
            if (error)
            {
                complectionBlock(nil, error);
            }else
            {
                complectionBlock(responseObject[AJNetManagerShare.resultKey], nil);
                // 对数据进行异步缓存
                [AJNetWorkCache setHttpCache:responseObject urlString:urlString parameters:parameters];
            }
            [[weakself tasks] removeObject:sessionTask];
    ...
    {
        sessionTask = [AJNetManagerShare.afSessionManager POST:URLString parameters:parameters progress:^(NSProgress * _Nonnull uploadProgress)
        {
            NetLog(@"上传进度--%lld, 总进度---%lld",uploadProgress.completedUnitCount,uploadProgress.totalUnitCount);
    ...
    {
        sessionTask = [AJNetManagerShare.afSessionManager PUT:URLString parameters:parameters success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject)
        {
            NSError *error = [AJNetManagerShare parseResultObject:responseObject responseError:nil];
            if (error)
            {
    ...
    {
        sessionTask = [AJNetManagerShare.afSessionManager DELETE:URLString parameters:parameters success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject)
        {
            NSError *error = [AJNetManagerShare parseResultObject:responseObject responseError:nil];
            if (error)
            {
...
/**
 从responseObject中解析error
 
 @param responseObject 服务器返回的错误信息,
 {
 "resultCode":0,  //0为成功，其他为失败
 "resultMsg":"success", //结果message
 "result":{
 结果返回数据，可能为 json or jsonArray
 }
 }
 @param error nil
 @return NSError
 */
- (NSError *)parseResultObject:(id __nullable) responseObject
                 responseError:(NSError* __nullable) error
{
    if ([responseObject isKindOfClass:[NSDictionary class]])
    {
        NSDictionary * dictErrors = (NSDictionary*)responseObject;
        
        id result = [dictErrors objectForKey:self.resultKey];
        NSString * errorMsg = [dictErrors objectForKey:self.megKey];
        NSString * code = [dictErrors objectForKey:self.codeKey];
        if (self.errorCode.count >0)
        {
            //根据服务器返回的code得出对应的message
            errorMsg = [self.errorCode objectForKey:self.codeKey];
        }
        
        if (errorMsg.length > 0)
        {
            return [NSError errorWithDomain:self.domainKey
